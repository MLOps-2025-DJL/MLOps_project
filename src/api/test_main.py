...
class TestMainPy(unittest.TestCase):
    ...
    def test_find_latest_model_no_pkl(self):
        ...
        mock_s3_client.list_objects_v2.return_value = {
            'Contents': [{'Key': 'data.csv', 'LastModified': datetime(2023, 3, 1)}]
        }
    ...
    def test_load_model(self):
        ...
        with patch('main.find_latest_model') as mock_find_latest:
            ...
            mock_s3_client.get_object.assert_called_once_with(
                Bucket='test-bucket', Key='model.pkl'
            )
    ...
    def test_predict_image(self):
        ...
        mock_model.predict.return_value = (
            'cat',
            None,
            MagicMock(max=lambda: MagicMock(item=lambda: 0.95))
        )
    ...
    def test_predict_endpoint_no_model(self):
        ...
        with patch('main.model', None), patch(
            'main.load_model', side_effect=Exception('Test error')
        ):
    ...
    def test_predict_endpoint_invalid_image(self):
        # Simuler un modèle chargé
        with patch('main.model', MagicMock()), patch(
            'PIL.Image.open', side_effect=IOError("cannot identify image file")
        ):
            ...
            # Faire une requête à l'endpoint
            response = client.post(
                "/predict", files={"file": ("invalid.jpg", invalid_data, "image/jpeg")}
            )
    ...
    def test_predict_endpoint_unexpected_model_output(self):
        ...
        with patch('PIL.Image.open', return_value=test_image), patch(
            'main.model'
        ) as mock_model:
